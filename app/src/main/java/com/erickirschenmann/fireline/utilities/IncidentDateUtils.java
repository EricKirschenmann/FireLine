...
public final class IncidentDateUtils {
   ...
   * local time zone. For example, if you live in California and the day is September 20th, 2016 and
   * it is 6:30 PM, it will return 1474329600000. Now, if you plug this number into an Epoch time
   * converter, you may be confused that it tells you this time stamp represents 8:00 PM on
   * September 19th local time, rather than September 20th. We're concerned with the GMT date here
   * though, which is correct, stating September 20th, 2016 at midnight.
   *
   * <p>As another example, if you are in Hong Kong and the day is September 20th, 2016 and it is
   * 6:30 PM, this method will return 1474329600000. Again, if you plug this number into an Epoch
   * time converter, you won't get midnight for your local time zone. Just keep in mind that we are
   * just looking at the GMT date here.
   *
   * <p>This method will ALWAYS return the date at midnight (in GMT time) for the time zone you are
   * currently in. In other words, the GMT date will always represent your date.
   *
   * <p>Since UTC / GMT time are the standard for all time zones in the world, we use it to
   * normalize our dates that are stored in the database. When we extract values from the database,
   * we adjust for the current time zone using time zone offsets.
   *
   ...
   *
   * <p>Normalize, in our usage within Sunshine means to convert a given date in milliseconds to the
   * very beginning of the date in UTC time.
   *
   * <p>For example, given the time representing
   *
   * <p>Friday, 9/16/2016, 17:45:15 GMT-4:00 DST (1474062315000)
   *
   * <p>this method would return the number of milliseconds (since the epoch) that represents
   *
   * <p>Friday, 9/16/2016, 00:00:00 GMT (1473984000000)
   *
   * <p>To make it easy to query for the exact date, we normalize all dates that go into the
   * database to the start of the day in UTC time. In order to normalize the date, we take advantage
   * of simple integer division, noting that any remainder is discarded when dividing two integers.
   *
   * <p>For example, dividing 7 / 3 (when using integer division) equals 2, not 2.333 repeating as
   * you may expect.
   *
   ...
   * users. As classy and polished a user experience as "1474061664" is, we can do better.
   *
   * <p>The day string for forecast uses the following logic: For today: "Today, June 8" For
   * tomorrow: "Tomorrow For the next 5 days: "Wednesday" (just the day name) For all days after
   * that: "Mon, Jun 8" (Mon, 8 Jun in UK, for example)
   *
   ...
   *     "Friday"
   */
  public static String getFriendlyDateString(
      Context context, long normalizedUtcMidnight, boolean showFullDate) {
  ...
  private static String getReadableDateString(Context context, long timeInMillis) {
    int flags =
        DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_NO_YEAR | DateUtils.FORMAT_SHOW_WEEKDAY;
