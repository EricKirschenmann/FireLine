...
import static com.erickirschenmann.fireline.data.TestUtilities.getConstantNameByStringValue;
...
public class TestIncidentDatabase {
  ...
  private static final String columnIncidentIdVariableName = "COLUMN_WEATHER_ID";
  ...
  public void before() {
    try {
      ...
      REFLECTED_COLUMN_WEATHER_ID =
          getStaticStringField(weatherEntryClass, columnIncidentIdVariableName);
  ...
  public void testDatabaseVersionWasIncremented() {
    ...
    String databaseVersionShouldBe1 =
  ...
  /**
   * Tests to ensure that inserts into your database results in automatically incrementing row IDs
   * and that row IDs are not reused.
   *
   * <p>If the INTEGER PRIMARY KEY column is not explicitly given a value, then it will be filled
   * automatically with an unused integer, usually one more than the largest _ID currently in use.
   * This is true regardless of whether or not the AUTOINCREMENT keyword is used.
   *
   * <p>If the AUTOINCREMENT keyword appears after INTEGER PRIMARY KEY, that changes the automatic
   * _ID assignment algorithm to prevent the reuse of _IDs over the lifetime of the database. In
   * other words, the purpose of AUTOINCREMENT is to prevent the reuse of _IDs from previously
   * deleted rows.
   *
  ...
  public void testDuplicateDateInsertBehaviorShouldReplace() {
    ...
    /*
     * Get the original weather ID of the testIncidentValues to ensure we use a different
     * weather ID for our next insert.
     */
    long originalIncidentId = testIncidentValues.getAsLong(REFLECTED_COLUMN_WEATHER_ID);
    ...
    testIncidentValues.put(REFLECTED_COLUMN_WEATHER_ID, newIncidentId);
  ...
  public void testNullColumnConstraints() {
    ...
    for (String columnName : weatherTableColumnNames) {
      ...
      String nullRowInsertShouldFail =
          "Insert should have failed due to a null value for column: '"
              + columnName
              + "'"
              + ", but didn't."
  ...
  public void testOnUpgradeBehavesCorrectly() {
    ...
    assertEquals(shouldHaveSingleTable, expectedTableCount, tableNameCursor.getCount());
  ...
  /**
   * This method tests inserting a single record into an empty table from a brand new database. It
  ...
  public void testInsertSingleRecordIntoIncidentTable() {
    ...
    /* Insert ContentValues into database and get a row ID back */
    long weatherRowId = database.insert(REFLECTED_TABLE_NAME, null, testIncidentValues);
    ...
    String expectedIncidentDidntMatchActual = "Expected weather values didn't match actual values.";
    TestUtilities.validateCurrentRecord(
        expectedIncidentDidntMatchActual, weatherCursor, testIncidentValues);
